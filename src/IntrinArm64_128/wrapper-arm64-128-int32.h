...
#define SHUFFLES(IMM) \
  static inline int32x4_t shiftLeft_n_##IMM##_s32(int32x4_t x) { \
    return vshlq_n_s32(x, IMM); \
  } \
  static inline int32x4_t shiftRight_n_##IMM##_s32(int32x4_t x) { \
    return vshrq_n_s32(x, IMM); \
  } \
  static inline uint32x4_t shiftRight_n_##IMM##_u32(uint32x4_t x) { \
    return vshrq_n_u32(x, IMM); \
  }
