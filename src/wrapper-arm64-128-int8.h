...
#define GET_LANE_16x8i(LANE) \
  static inline int8_t get_lane_16x8i##LANE(int8x16_t v) { \
...
#define SET_LANE_16x8i(LANE) \
  static inline int8x16_t set_lane_16x8i##LANE(int8x16_t v, int8_t x) { \
    return vsetq_lane_s8(x, v, LANE); \
  }
LANES(SET_LANE_16x8i)
#undef SET_LANE_16x8i

static inline int8x16_t load16x8i(const int8_t* x) { return vld1q_s8(x); }
static inline void store16x8i(int8_t* x, int8x16_t y) { vst1q_s8(x, y); }

#define EXTRACT_VECTOR_16x8i(LANE) \
  static inline int8x16_t extractVector16x8i##LANE(int8x16_t x, int8x16_t y) { \
